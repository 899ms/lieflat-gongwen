...
**句长与并列**。公文平均句长 53 字，是自媒体的 2 倍；顿号密度每千字 16-27 个。实现方式是**并列成分堆叠**：
...
工作方案是八个文种里顿号最疏的一档（18‰），把任务写成四五项器材清单就会顶破。经验总结相反，是句长最长的一档（真件实测 68.9–75.0），而且它的要求词多半来自**"确保"而不是"要"**，盯错词就补不上去。
...
**站位铁律**：226 篇统计中，以制度缺陷作为开篇切入点的**仅 2 篇**。要批评体制，前面必须先垫战略高度和已有成效——想批评，先把资格挣到手。详见 `公文语料/写作视角与认知框架.md`。
...
**统计口径**：篇章层面的全量统计（收尾模式、开头策略、站位选择、重心分布）跑的是 229/226 篇的完整公文语料，其中含 38 篇已移除的心得体会。按文种分列的参数不含它，被删的是整行；聚合数字里仍含它的贡献，原文已移除、无法重算。
